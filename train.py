...
def label_contrastive_loss(features, labels, temperature):
    ...
    denominator = exp_sim.sum(dim=1) - torch.exp(torch.diag(similarity))
    loss = -torch.log((numerator + 1e-8) / (denominator + 1e-8))
    return loss.mean()


def translation_contrastive_loss(features_anchor, features_positive, temperature):
    features_anchor = F.normalize(features_anchor, p=2, dim=1)
    features_positive = F.normalize(features_positive, p=2, dim=1)
    similarity = torch.matmul(features_anchor, features_positive.T) / temperature
    positive_sim = torch.exp(torch.diag(similarity))
    denominator = torch.exp(similarity).sum(dim=1)
    loss = -torch.log((positive_sim + 1e-8) / (denominator + 1e-8))
    return loss.mean()
...
def train(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (__, __, blocks) in enumerate(dataloader):
            ...
            h, batch_pred = model(dataset.g, blocks, batch_inputs)


            #print(batch_pred.size())
            #print(batch_labels.size())
            # loss = loss_fcn(batch_pred, batch_labels)
            loss_ce = loss_fcn(batch_pred, batch_labels)
            current_gamma = args.gamma0 * math.exp(-args.gamma_decay * global_step)
            doc_reps = h[DOC_NODE_TYPE]  # (batch_size, d)
            loss_llcl = label_contrastive_loss(doc_reps, batch_labels, current_gamma)
            loss_tlcl = 0.0
            if "trans" in batch_inputs:
                anchor_reps = batch_inputs[DOC_NODE_TYPE]
                positive_reps = batch_inputs["trans"]
                loss_tlcl = translation_contrastive_loss(anchor_reps, positive_reps, current_gamma)
            total_loss = loss_ce + args.lambda_contrast * loss_llcl + args.lambda_tl * loss_tlcl
            ...
            if (step + 1) % args.log_every == 0:
                ...
                logger.info(f"Epoch {epoch} | Step {step + 1:02d} | lr {optimizer.param_groups[0]['lr']:.2e} "
                            f"| CE Loss {loss_ce.item():.4f} | LL-CL Loss {loss_llcl.item():.4f} | TL-CL Loss {loss_tlcl if isinstance(loss_tlcl, float) else loss_tlcl.item():.4f} "
                            f"| Total Loss {total_loss.item():.4f} | Train Acc {acc.item():.4f} "
                            f"| Temp {current_gamma:.4f} | Speed (samples/sec) {np.mean(iter_tput[-args.log_every:]):.4f}")
                # logger.info(f"Epoch {epoch} | Step {step + 1:02d} | lr {optimizer.param_groups[0]['lr']:.2e}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lambda_contrast", type=float, default=1.0, help="Weight for label contrast loss (LL-CL)")
    parser.add_argument("--lambda_tl", type=float, default=0.0, help="Weight for translation contrast loss (TL-CL)")
    parser.add_argument("--gamma0", type=float, default=0.5, help="Initial temperature for contrast loss")
    parser.add_argument("--gamma_decay", type=float, default=1e-4, help="Decay rate for temperature")
