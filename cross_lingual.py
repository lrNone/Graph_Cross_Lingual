...
#            for trans_pair in [(self.src_lan, self.tgt_lan)]:
    ...
    def _process_doc_doc_edges(self) -> dict:
        """build edges between bilingual sentences"""
        doc_doc_edge = {DOC_SIM_EDGE_TYPE: [], DOC_TRANS_EDGE_TYPE: []}
        sim_edges = doc_doc_edge[DOC_SIM_EDGE_TYPE]
        trans_edges = doc_doc_edge[DOC_TRANS_EDGE_TYPE]
        ...
        if hasattr(self, "source_ids") and hasattr(self, "target_ids"):
            threshold = 0.8  # 相似度阈值
            for s in self.source_ids:
                for t in self.target_ids:
                    sim_val = torch.nn.functional.cosine_similarity(
                        torch.Tensor([self.text_embeddings[s]]).to(self.device),
                        torch.Tensor([self.text_embeddings[t]]).to(self.device)
                    ).item()
                    if sim_val > threshold:
                        sim_edges.append((s, t))
                        sim_edges.append((t, s))
